...
ImageInfo* SFBMCache::loadPattern(std::string smatcherID, int pidx) {
    ...
    if (smatcherPatterns->find(pidx) == smatcherPatterns->end()) {
        this->dbdriver->retrieveNthPattern(smatcherID, pidx, &pattern);
...
JSONValue* SFBMCache::getLastOperationResult() {
    ...
    switch (operation) {
        case SFBMCache::INSERTOP : {
            result = this->rw->resultAsJSONValue(lastInsertedIndex, this->cacheMaxSize - this->cacheSize, lastRemovedIndex);
            break;
        }
        case SFBMCache::DELETEOP : {
            result = this->rw->resultAsJSONValue(lastRemovedIndex, this->cacheMaxSize - this->cacheSize);
            break;
        }
        case SFBMCache::UPDATEOP : {
            result = this->rw->resultAsJSONValue(lastInsertedIndex);
            break;
        }
    }
    return result;
