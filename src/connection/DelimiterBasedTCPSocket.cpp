...
DelimiterBasedTCPSocket::DelimiterBasedTCPSocket(std::string ip,
		std::string port, std::string ld, std::string md, char sendType) :
...
DelimiterBasedTCPSocket::DelimiterBasedTCPSocket(tcp::socket sock,
		std::string ld, std::string md, char sendType) :
...
void DelimiterBasedTCPSocket::bsend(std::string msg) {
	...
	if (showComs)
		std::cout << "sent " << request << " to "
				<< boost::lexical_cast<std::string>(s.remote_endpoint())
				<< std::endl;
}
...
std::string DelimiterBasedTCPSocket::bread(int lenght) {
	...
	while (!stop) {
		std::string lmsg;
		lmsg = read_until(this->lineDelimiter, this->messageDelimiter, &stop,
				&error);
		if (error) {
