...
AUDPSocket::AUDPSocket(std::string ip, std::string port, char sendType) :
		s(
				io_service/*, udp::endpoint(udp::v4(), boost::lexical_cast<int>(port))*/) {
...
AUDPSocket::AUDPSocket(unsigned short port, char sendType) :
		s(io_service, udp::endpoint(udp::v4(), port)) {
	this->sendType = sendType;
	//this->s.open(udp::v4());
}
...
bool AUDPSocket::connect() {
	if (showComs)
		cout << "Connecting to " << this->ip << ":" << this->port << endl;
	send(this->ip);
	send(boost::lexical_cast<std::string>(this->s.local_endpoint().port()));
	string response = read();
	if (showComs)
		cout << "Remote host responded with : " << response << endl;
	if (response == "OK") {
		if (showComs)
			cout << "Connected to " << this->ip << ":" << this->port << endl;
		return true;
	}
	if (showComs)
...
bool AUDPSocket::waitForConnection() {
	...
	if (showComs)
		cout << "Connection data received: " << fullMsg << endl;
	remote_endpoint = boost::asio::ip::udp::endpoint(
			boost::asio::ip::address::from_string(ip),
			boost::lexical_cast<int>(port));
	this->ip = ip;
	...
	send("OK");
	if (showComs)
		cout << "Connected with: " << lastSender << endl;
	return true;
}
...
void AUDPSocket::bsend(string msg) {
	...
	this->s.send_to(boost::asio::buffer(request, request_length),
			this->remote_endpoint); //boost::asio::write(s, boost::asio::buffer(request, request_length));
	if (showComs)
		cout << "sent " << request << " to "
				<< boost::lexical_cast<std::string>(this->remote_endpoint)
				<< endl;
}
...
string AUDPSocket::bread(int lenght) {
	...
	if (showComs)
		cout << "received " << result << " from "
				<< boost::lexical_cast<std::string>(sender_endpoint) << endl;
	return result;
}
