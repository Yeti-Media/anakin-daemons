...
DTCPServerSocket::DTCPServerSocket(unsigned short port,
		std::string lineDelimiter, std::string messageDelimiter) :
		ATCPServerSocket(port) {
...
Socket* DTCPServerSocket::waitForConnection() {
	...
	if (showComs)
		std::cout << "accepted a connection from "
				<< boost::lexical_cast<std::string>(sock.remote_endpoint())
				<< std::endl;
	Socket* s = new DelimiterBasedTCPSocket(std::move(sock),
			this->lineDelimiter, this->messageDelimiter);
	return s;
}
