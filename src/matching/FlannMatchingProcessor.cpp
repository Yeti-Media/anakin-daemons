...
std::vector<JSONValue*>* FlannMatchingProcessor::process(RichImg* scene, bool * error)
{
    ...
    for (uint m = 0; m < matches->size() && !*error; m++)
    {
        ...
        sceneResult->push_back(this->rw->matchAsJSON(match.getCenter(), match.getPattern()->getImage()->getLabel(), match.getMatchedKeypoints()));
    }
    return sceneResult;
}
