...
std::vector<Match>* BasicFlannDetector::findPatterns(RichImg* scene, bool * error)
{
...
vector<Anakin::Match>* BasicFlannDetector::findPatterns_usingTraining(Anakin::RichImg* scene, bool * error)
{
    ...
    for (uint pm = 0; pm < matchedImgsIdx->size(); pm++)
    {
        ...
        if (pattern_matches->size() >= this->min_matches_allowed)
        {
            ...
            ImageInfo* pii = this->cache->loadPattern(trainerID, currentKey, error);
            if (*error) break;
