...
SerializableFlannBasedMatcher::SerializableFlannBasedMatcher(
		std::string filename, bool removeFileAfterLoad) {
...
void SerializableFlannBasedMatcher::save(std::string filename,
		std::string * xmlData) {
	this->filename = filename;
	...
	mdptr++;
	mdptr =
			(cv::DescriptorMatcher::DescriptorCollection*) ((std::vector<int> *) mdptr
					- 1);
...
void SerializableFlannBasedMatcher::compress(bool removeOriginal) {
	...
	std::string compressedIndexFileName =
			removeOriginal ?
					uncompressedIndexFileName : this->filename + ".cif";
	std::string compressedMatcherFileName =
			removeOriginal ?
	...
	qlz_state_compress *state_compress_index = (qlz_state_compress *) malloc(
			sizeof(qlz_state_compress));
	qlz_state_compress *state_compress_matcher = (qlz_state_compress *) malloc(
			sizeof(qlz_state_compress));
	int ir;
	int mr;
	//clock_t t_1 = clock();
	ir = qlz_compress(uindexData, compressedIndexData, uindexDataSize,
			state_compress_index);
	mr = qlz_compress(umatcherData, compressedMatcherData, umatcherDataSize,
			state_compress_matcher);
	//clock_t t_2 = clock();
...
void SerializableFlannBasedMatcher::decompress(bool useOriginalNames,
		std::string * xmlData) {
	bool decompressMatcherToFile = xmlData == NULL;
	std::string uncompressedIndexFileName = this->filename
			+ (useOriginalNames ? ".if" : ".uif");
	std::string uncompressedMatcherFileName = this->filename
			+ (useOriginalNames ? ".xml" : ".uxml");
	std::string compressedIndexFileName = this->filename
			+ (useOriginalNames ? ".if" : ".cif");
	std::string compressedMatcherFileName = this->filename
	...
	qlz_state_decompress *state_decompress_index =
			(qlz_state_decompress *) malloc(sizeof(qlz_state_decompress));
	qlz_state_decompress *state_decompress_matcher =
			(qlz_state_decompress *) malloc(sizeof(qlz_state_decompress));
	int ir;
	int mr;
	//clock_t t_1 = clock();
	ir = qlz_decompress(cindexData, ucompressedIndexData,
			state_decompress_index);
	mr = qlz_decompress(cmatcherData, ucompressedMatcherData,
			state_decompress_matcher);
	//clock_t t_2 = clock();
	//float tt = ((float)(t_2 - t_1))/CLOCKS_PER_SEC;
...
int SerializableFlannBasedMatcher::readFile(char ** data,
		std::string filename) {
...
void SerializableFlannBasedMatcher::writeFile(char * data, std::string filename,
		int length) {
