...
bool DBDriver::saveUser(DBUser* u) {
	if (checkConn()) {
		...
		this->lastMessageReceived =
				this->lastMessageReceived.append("User ").append(
						std::to_string(u->getID())).append(
						" saved successfully");
		PQclear(res);
		return true;
	} else {
...
bool DBDriver::retrieveUser(int id, bool * error, bool load, DBUser** result,
	...
	*error = false;
	if (checkConn()) {
		...
		this->lastMessageReceived =
				this->lastMessageReceived.append("User with id ").append(
						std::to_string(id)).append(" found").append(
						load ? " and loaded" : "");
		PQclear(res);
		return true;
	} else {
...
bool DBDriver::saveUserPatterns(DBUser* u, bool saveNeededObjectsFirst) {
	...
	if (!userExist && !error) {
		if (saveNeededObjectsFirst) {
			...
			this->lastMessageReceived = this->lastMessageReceived.append(
					"User ").append(std::to_string(u->getID())).append(
					" doesn't exist");
			return false;
		}
	} else if (error) {
	...
	if (checkConn()) {
		...
		if (!userExist && !error) {
			if (saveNeededObjectsFirst) {
				...
				this->lastMessageReceived = "User " + suid + " doesn't exist";
				return false;
			}
		} else if (error) {
			return false;
		...
		PGresult *res;
		const char *paramValues[4] = { spid, colorData.c_str(),
		...
		this->lastMessageReceived =
				this->lastMessageReceived.append("User ").append(
						std::to_string(h->getUserID())).append(" now has ").append(
						object).append(" ").append(std::to_string(h->getID()));
		PQclear(res);
		return true;
	} else {
		return false;
...
bool DBDriver::retrievePattern(int id, bool * error, bool load,
	...
	*error = false;
	if (checkConn()) {
		...
		this->lastMessageReceived = basicInfo
				+ (descInfo.empty() ? "" : ("\n" + descInfo));
		return true;
	} else {
		*error = true;
...
bool DBDriver::storeSFBM(std::string filename, int * smatcher_id, int userID,
	...
	if (checkConn()) {
		...
		const char *paramValues[3] = { matcher_id_svalue.c_str(),
		...
		*smatcher_id = std::stoi(stid);
		...
		this->lastMessageReceived = "succesfully uploaded " + matcher_filename
				+ " and " + index_filename + " to db";
	}
...
bool DBDriver::retrieveSFBM(int smatcher_id, bool * error) {
	*error = false;
	if (checkConn()) {
		...
		if (PQntuples(res) == 0) {
			this->lastMessageReceived = "No serialized matcher found with id: "
					+ smatcher_id;
			return false;
		}
		...
		if (indexFileLoaded && matcherFileLoaded) {
			this->lastMessageReceived = "Trainer " + filename
...
bool DBDriver::retrieveNthPattern(int smatcher_id, int pidx,
	...
	*error = false;
	if (checkConn()) {
		...
		std::string scategory = std::to_string(category);
		const char *paramValues[3] = { sid.c_str(), spidx.c_str(),
				scategory.c_str() };
		std::string table;
		table =
				table.append("public.\"").append(Constants::PATTERN_TABLE).append(
		...
		if (PQntuples(res) == 0) {
			this->lastMessageReceived = "No pattern found for smatcher : " + sid
					+ " and index " + spidx;
			return false;
		}
		...
		*pattern = ii;
		this->lastMessageReceived = spidx + "-th Pattern of smatcher " + sid
				+ " loaded";
		return true;
	} else {
		*error = true;
...
bool DBDriver::savePatternDescriptors(int id, std::string data) {
	...
	this->lastMessageReceived = "Descriptors for pattern " + sid
			+ " saved succesfully";
	PQclear(res);
	return true;
}
...
bool DBDriver::getPatternBasicInfo(int id, int * user_id, bool * error) {
	...
	*user_id = std::stoi(suid);
	this->lastMessageReceived = "Basic information for pattern " + sid
			+ " loaded";
	return true;
}
...
bool DBDriver::updatePatternTrainerInfo(int id, int trainer_id, int position) {
	...
	this->lastMessageReceived = "Updated pattern " + sid + " with trainer "
			+ stid + " and position " + spos;
	return true;
}
...
bool DBDriver::retrieveHORL(int id, char mode, bool * error, bool load,
	...
	*error = false;
	if (checkConn()) {
		...
		if (PQntuples(res) == 0) {
			this->lastMessageReceived = "No " + object + " found with id: "
					+ ssid;
			return false;
		}
		if (load) {
		...
		this->lastMessageReceived = object + " with id " + ssid + " found"
				+ (load ? " and loaded" : "");
		PQclear(res);
		return true;
	} else {
