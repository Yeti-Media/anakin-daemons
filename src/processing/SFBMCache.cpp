...
SFBMCache::SFBMCache(DBDriver* dbdriver, int loadingTimeWeight, bool discardLessValuable, int cacheSize, int life, int scenesCacheSize, int scenesLife)
{
...
void SFBMCache::updateMatcher(int smatcher_id, bool * error)
{
    ...
    if (!*error) this->operation = UPDATEOP;
}
...
ImageInfo* SFBMCache::loadPattern(int smatcherID, int pidx, bool * error)
{
    ...
    {
        this->dbdriver->retrieveNthPattern(smatcherID, pidx, &pattern, error);
        if (!*error) (*smatcherPatterns)[pidx] = pattern;
    }
    if (!*error) pattern = smatcherPatterns->find(pidx)->second;
    sem_post(&this->sem);
...
JSONValue* SFBMCache::getLastOperationResult()
{
    JSONValue* result;
    switch (operation)
    {
    ...
    case SFBMCache::DBERROR :
    {
        //get error message using dbdriver->lastMessageReceived
        break;
    }
    }
    return result;
...
SerializableFlannBasedMatcher* SFBMCache::loadMatcherFromDB(int smatcher_id, float* loadingTime, bool * error)
{
    ...
    clock_t t_1 = clock();
    this->dbdriver->retrieveSFBM(smatcher_id, error);
...
{
    ImageInfo* scene;
    this->dbdriver->retrieveScene(&scene, sceneID, error);
    return (!*error)?scene:NULL;
}
