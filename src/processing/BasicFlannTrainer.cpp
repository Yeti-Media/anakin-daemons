...
void BasicFlannTrainer::train_and_save() {
    ...
    //this->detector->train();
    ((Ptr<SerializableFlannBasedMatcher>)this->detector)->save(this->outputFolder+this->fileName);
}


BasicFlannTrainer::BasicFlannTrainer(   cv::Ptr<SerializableFlannBasedMatcher> detector,
                                        std::vector<Anakin::RichImg*>& patterns,
                                        std::string outputFolder,
                                        std::string fileName) : Trainer(detector, patterns, outputFolder, fileName){
}
