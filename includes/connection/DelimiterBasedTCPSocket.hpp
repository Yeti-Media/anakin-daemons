...
public:
	 ...
	 */
	DelimiterBasedTCPSocket(std::string ip, std::string port, std::string ld,
			std::string md, char sendType = DEFAULTSEND);
	/**
	 ...
	 * sendType : synchonized or default send (see Socket)
	 */
	DelimiterBasedTCPSocket(tcp::socket sock, std::string ld, std::string md,
			char sendType = DEFAULTSEND);
...
protected:
	 ...
	 */
	std::string read_until(std::string ld, std::string md, bool * eom,
	bool * error);
private:
};
}
