...
* How to apply valgrind to the full anakin acceptance testing?
        1) compile anakin in COMPILE_FOR_BIN_ACCEPTANCE_TESTING mode.
        2) run valgrind using the args:
                valgrind --leak-check=yes --log-file="report.txt" --track-origins=yes --suppressions="valgrindSupress.txt"

* How to add more filters to valgrind false positives?
        run this command, and follow the steps
                valgrind --leak-check=yes --track-origins=yes --gen-suppressions=yes --suppressions="valgrindSupress.txt" 
