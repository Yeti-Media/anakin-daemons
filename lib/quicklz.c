...
static size_t qlz_compress_core(const unsigned char *source,
		unsigned char *destination, size_t size, qlz_state_compress *state) {
	...
	const unsigned char *last_matchstart = last_byte - UNCONDITIONAL_MATCHLEN
			- UNCOMPRESSED_END;
	ui32 fetch = 0;
	unsigned int lits = 0;
...
#elif QLZ_COMPRESSION_LEVEL >= 2
		{
			...
			size_t remaining =
					(last_byte - UNCOMPRESSED_END - src + 1) > 255 ?
							255 : (last_byte - UNCOMPRESSED_END - src + 1);
			(void) best_k;
			...
			for (k = 1; k < QLZ_POINTERS && c > k; k++) {
				...
				if (((fast_read(o, 3) ^ fetch) & 0xffffff)
						== 0&& o < src - MINOFFSET)
#elif QLZ_COMPRESSION_LEVEL == 2
...
static size_t qlz_decompress_core(const unsigned char *source,
		unsigned char *destination, size_t size, qlz_state_decompress *state,
	...
	const unsigned char *last_matchstart = last_destination_byte
			- UNCONDITIONAL_MATCHLEN - UNCOMPRESSED_END;
	unsigned char *last_hashed = destination - 1;
	const unsigned char *last_source_byte = source
			+ qlz_size_compressed((const char *) source) - 1;
	static const ui32 bitlut[16] = { 4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0,
			1, 0 };
...
size_t qlz_compress(const void *source, char *destination, size_t size,
...
#endif
	{
		...
		r = base
				+ qlz_compress_core((const unsigned char *) source,
	...
	*destination |=
			((QLZ_STREAMING_BUFFER == 0 ?
					...
					(QLZ_STREAMING_BUFFER == 100000 ?
							1 : (QLZ_STREAMING_BUFFER == 1000000 ? 2 : 3))) << 4);
