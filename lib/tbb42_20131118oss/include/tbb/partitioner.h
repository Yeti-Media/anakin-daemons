...
struct partition_type_base {
	...
	flag_task* split_work(StartType &start) {
		...
		StartType& right_work = *new (parent_ptr->allocate_child()) StartType(
				start, split());
	...
	void execute(StartType &start, Range &range) {
		...
		if (range.is_divisible()) {
			if (derived().divisions_left())
				do
					parent_ptr = split_work(start); // split until divisions_left()
			...
			if (derived().should_create_trap()) { // only for range pool
				if (parent_ptr->ref_count() > 1) { // create new parent if necessary
					...
					__TBB_ASSERT(parent_ptr->ref_count() == 1, NULL);
				parent_ptr->set_ref_count(2); // safe because parent has only one reference
				signal_task& right_signal =
						*new (parent_ptr->allocate_child()) signal_task();
		...
		if (!range.is_divisible() || !derived().max_depth())
			...
			internal::range_vector<Range, Partition::range_pool_size> range_pool(
					range);
			do {
				...
				if (derived().check_for_demand(start)) {
					if (range_pool.size() > 1) {
						...
						start.set_parent(parent_ptr);
						parent_ptr->set_ref_count(2);
						StartType& right_work = *new (
								parent_ptr->allocate_child()) StartType(start,
								range_pool.front(), range_pool.front_depth());
						start.spawn(right_work);
...
struct auto_partition_type_base: partition_type_base<Partition> {
	...
	auto_partition_type_base() :
			my_max_depth(__TBB_INIT_DEPTH) {
		my_divisor = tbb::internal::get_initial_auto_partitioner_divisor()
				* __TBB_INITIAL_CHUNKS / 4;
		__TBB_ASSERT(my_divisor,
				"initial value of get_initial_auto_partitioner_divisor() is not valid");
	}
	auto_partition_type_base(auto_partition_type_base &src, split) {
		...
		if (my_divisor)
			src.my_max_depth += static_cast<depth_t>(__TBB_Log2(
...
class affinity_partition_type: public auto_partition_type_base<
...
public:
	...
	affinity_partition_type(affinity_partition_type& p, split) :
			auto_partition_type_base<affinity_partition_type>(p, split()), my_array(
					p.my_array) {
		__TBB_ASSERT(
				p.map_end-p.map_begin<factor || (p.map_end-p.map_begin)%factor==0,
				NULL);
		map_end = p.map_end;
...
public:
	...
	old_auto_partition_type(const auto_partitioner&) :
			num_chunks(
					internal::get_initial_auto_partitioner_divisor()
							* __TBB_INITIAL_CHUNKS / 4) {
	}
