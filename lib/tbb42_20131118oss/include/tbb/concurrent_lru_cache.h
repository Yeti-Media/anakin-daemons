...
private:
	...
	struct map_value_type {
		...
		map_value_type(value_type const& a_value,
				...
				bool a_is_ready) :
				my_value(a_value), my_ref_counter(a_ref_counter), my_lru_list_iterator(
						a_lru_list_iterator), my_is_ready(a_is_ready) {
		}
	};
	...
	friend class tbb::internal::aggregating_functor<self_type,
			aggregated_operation_type>;
	typedef tbb::internal::aggregator<aggregator_function_type,
			aggregated_operation_type> aggregator_type;
...
public:
	concurrent_lru_cache(value_function_type f,
			...
			my_value_function(f), my_number_of_lru_history_items(
...
private:
	...
	public:
		...
		handle_object(handle_move_t m) :
				my_cache_pointer(&m.my_cache_ref), my_map_record_ref(
						m.my_map_record_ref) {
		}
	...
	private:
		...
		static handle_move_t move(handle_object& h) {
			__TBB_ASSERT(h.my_cache_pointer,
...
private:
	...
	struct aggregator_operation: tbb::internal::aggregated_operation<
			aggregator_operation> {
		enum e_op_type {
			op_retive, op_signal_end_of_usage
		};
	...
	struct retrieve_aggregator_operation: aggregator_operation,
		...
		retrieve_aggregator_operation(key_type key) :
				aggregator_operation(aggregator_operation::op_retive), my_key(
						key), my_is_new_value_needed(false) {
		}
		void handle(self_type& container) {
			my_result_map_record_pointer = &container.retrieve_serial(my_key,
	...
	struct signal_end_of_usage_aggregator_operation: aggregator_operation,
		...
		signal_end_of_usage_aggregator_operation(
				...
				aggregator_operation(
						aggregator_operation::op_signal_end_of_usage), my_map_record_ref(
						map_record_ref) {
		}
		void handle(self_type& container) {
...
private:
	typename map_storage_type::reference retrieve_serial(key_type k,
			bool& is_new_value_needed) {
		typename map_storage_type::iterator it = my_map_storage.find(k);
		if (it == my_map_storage.end()) {
			it = my_map_storage.insert(it,
					std::make_pair(k,
	...
	void signal_end_of_usage_serial(
			typename map_storage_type::reference map_record_ref) {
		typename map_storage_type::iterator it = my_map_storage.find(
				map_record_ref.first);
		...
		__TBB_ASSERT(
				my_lru_list.end()== std::find(my_lru_list.begin(),my_lru_list.end(),it),
		...
		if (!--(it->second.my_ref_counter)) {
			...
			if (my_lru_list.size() >= my_number_of_lru_history_items) {
				...
				size_t number_of_elements_to_evict = 1 + my_lru_list.size()
