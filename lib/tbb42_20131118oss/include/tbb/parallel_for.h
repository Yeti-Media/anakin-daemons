...
public:
	...
	start_for(start_for& parent_, split) :
			my_range(parent_.my_range, split()), my_body(parent_.my_body), my_partition(
	...
	start_for(start_for& parent_, const Range& r, depth_t d) :
			my_range(r), my_body(parent_.my_body), my_partition(
					parent_.my_partition, split()) {
		my_partition.set_affinity(*this);
	...
	static void run(const Range& range, const Body& body,
		...
		if (!range.empty()) {
			...
			start_for& a = *new (task::allocate_root(context)) start_for(range,
					body, partitioner);
...
void parallel_for(const Range& range, const Body& body,
	...
	internal::start_for<Range, Body, const simple_partitioner>::run(range, body,
			partitioner);
}
...
void parallel_for(const Range& range, const Body& body,
	...
	internal::start_for<Range, Body, const auto_partitioner>::run(range, body,
			partitioner);
}
...
void parallel_for(const Range& range, const Body& body,
	...
	internal::start_for<Range, Body, affinity_partitioner>::run(range, body,
			partitioner);
}
...
void parallel_for(Index first, Index last, const Function& f) {
	parallel_for_impl<Index, Function, const auto_partitioner>(first, last,
			static_cast<Index>(1), f, auto_partitioner());
}
...
void parallel_for(Index first, Index last, Index step, const Function& f,
	...
	parallel_for_impl<Index, Function, const auto_partitioner>(first, last,
			step, f, auto_partitioner(), context);
}
...
void parallel_for(Index first, Index last, const Function& f,
		tbb::task_group_context &context) {
	parallel_for_impl<Index, Function, const auto_partitioner>(first, last,
			static_cast<Index>(1), f, auto_partitioner(), context);
}
