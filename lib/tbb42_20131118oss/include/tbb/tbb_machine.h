...
#error Unexpected value of __TBB_ENDIANNESS
	...
	volatile uint32_t * const aligned_ptr = (uint32_t*) ((uintptr_t) ptr
			- byte_offset);

	// location of T within uint32_t for a C++ shift operation
	const uint32_t bits_to_shift = 8
			* (endianness::is_big_endian() ?
					(4 - sizeof(T) - (byte_offset)) : byte_offset);
	...
	const uint32_t shifted_comparand = ((uint32_t) comparand << bits_to_shift)
			& mask;
...
template<>
inline uint16_t __TBB_CompareAndSwapGeneric<2, uint16_t>(volatile void *ptr,
		uint16_t value, uint16_t comparand) {
#if __TBB_USE_GENERIC_PART_WORD_CAS
...
template<>
inline uint32_t __TBB_CompareAndSwapGeneric<4, uint32_t>(volatile void *ptr,
		uint32_t value, uint32_t comparand) {
	// Cast shuts up /Wp64 warning
...
template<>
inline uint64_t __TBB_CompareAndSwapGeneric<8, uint64_t>(volatile void *ptr,
		uint64_t value, uint64_t comparand) {
...
struct machine_load_store_seq_cst {
	...
	static void store(volatile T &location, T value) {
		atomic_selector<S>::fetch_store((volatile void*) &location,
...
template<typename T>
const T reverse<T>::byte_table[256] = { 0x00, 0x80, 0x40, 0xC0, 0x20, 0xA0,
