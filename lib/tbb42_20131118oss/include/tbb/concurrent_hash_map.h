...
public:
	...
	bool check_rehashing_collision(const hashcode_t h, hashcode_t m_old,
			hashcode_t m) const {
		...
		if ((h & m_old) != (h & m)) { // mask changed for this hashcode, rare event
			...
			for (++m_old; !(h & m_old); m_old <<= 1)
				// at maximum few rounds depending on the first block size
				;
			m_old = (m_old << 1) - 1; // get full mask from a bit
			__TBB_ASSERT((m_old&(m_old+1))==0 && m_old <= m, NULL);
			// check whether it is rehashing/ed
			if (itt_load_word_with_acquire(get_bucket(h & m_old)->node_list)
					!= rehash_req) {
#if __TBB_STATISTICS
	...
	segment_index_t insert_new_node(bucket *b, node_base *n, hashcode_t mask) {
		...
		if (sz >= mask) { // TODO: add custom load_factor
			...
			if (!itt_hide_load_word(
					my_table[new_seg]) && as_atomic(my_table[new_seg]).compare_and_swap(is_allocating, NULL) == NULL)
	...
	void internal_swap(hash_map_base &table) {
		...
		for (size_type i = 0; i < embedded_buckets; i++)
			std::swap(this->my_embedded_segment[i].node_list,
...
public:
	...
	hash_map_iterator(
			...
			my_map(other.my_map), my_index(other.my_index), my_bucket(
					other.my_bucket), my_node(other.my_node) {
	}
...
hash_map_iterator<Container, Value>::hash_map_iterator(const Container &map,
		size_t index, const bucket *b, node_base *n) :
		my_map(&map), my_index(index), my_bucket(b), my_node(
				static_cast<node*>(n)) {
...
public:
	...
	hash_map_range(hash_map_range<U>& r) :
			my_begin(r.my_begin), my_end(r.my_end), my_midpoint(r.my_midpoint), my_grainsize(
					r.my_grainsize) {
	}
#if TBB_DEPRECATED
	...
	hash_map_range(const map_type &map, size_type grainsize_ = 1) :
			my_begin(
					Iterator(map, 0, map.my_embedded_segment,
							map.my_embedded_segment->node_list)), my_end(
					Iterator(map, map.my_mask + 1, 0, 0)), my_grainsize(
					grainsize_) {
		__TBB_ASSERT(grainsize_>0, "grainsize must be positive");
		set_midpoint();
	}
...
protected:
	...
	void rehash_bucket(bucket *b_new, const hashcode_t h) {
		...
		restart: for (node_base **p = &b_old()->node_list, *n =
				__TBB_load_with_acquire(*p); is_valid(n); n = *p) {
			hashcode_t c = my_hash_compare.hash(
					static_cast<node*>(n)->item.first);
#if TBB_USE_ASSERT
			...
			if ((c & mask) == h) {
				if (!b_old.is_writer())
					if (!b_old.upgrade_to_writer()) {
						goto restart;
						// node ptr can be invalid due to concurrent erase
					}
...
public:
	...
	template<typename I>
	concurrent_hash_map(I first, I last, const allocator_type &a =
			allocator_type()) :
			my_allocator(a) {
...
#if __TBB_INITIALIZER_LISTS_PRESENT
	//! Construct empty table with n preallocated buckets. This number serves also as initial concurrency level.
	concurrent_hash_map(const std::initializer_list<value_type> &il,
			const allocator_type &a = allocator_type()) :
			my_allocator(a) {
		reserve(il.size());
		...
		if (itt_load_word_with_acquire(b->node_list) == internal::rehash_req) {
			...
			if (lock.try_acquire(b->mutex, /*write=*/true)) {
				if (b->node_list == internal::rehash_req)
					const_cast<concurrent_hash_map*>(this)->rehash_bucket(b,
							h & m); //recursive rehashing
			} else
...
template<typename Key, typename T, typename HashCompare, typename A>
bool concurrent_hash_map<Key, T, HashCompare, A>::lookup(bool op_insert,
		const Key &key, const T *t, const_accessor *result, bool write) {
	...
	restart: { //lock scope
		...
		if (op_insert) {
			...
			if (!n) {
				if (!tmp_n) {
					if (t)
						tmp_n = new (my_allocator) node(key, *t);
					else
						tmp_n = new (my_allocator) node(key);
				}
			...
			if (!n) {
				if (check_mask_race(h, m))
					goto restart;
				// b.release() is done in ~b(). TODO: replace by continue
				return false;
			}
			return_value = true;
		}
...
template<typename Key, typename T, typename HashCompare, typename A>
template<typename I>
std::pair<I, I> concurrent_hash_map<Key, T, HashCompare, A>::internal_equal_range(
		const Key& key, I end_) const {
	hashcode_t h = my_hash_compare.hash(key);
...
bool concurrent_hash_map<Key, T, HashCompare, A>::erase(const Key &key) {
	...
	restart: { //lock scope
			   // get bucket
		bucket_accessor b(this, h & m);
		search: node_base **p = &b()->node_list;
		n = *p;
		while (is_valid(n)
...
void concurrent_hash_map<Key, T, HashCompare, A>::rehash(size_type sz) {
	...
	for (; b <= mask; b++, bp++) {
		...
		__TBB_ASSERT(
				is_valid(n) || n == internal::empty_rehashed || n == internal::rehash_req,
		...
		for (hashcode_t k = 0; k <= mask; k++) {
			...
			if (n == internal::rehash_req) { // source is not rehashed, items are in previous buckets
				...
				for (; n; n = static_cast<node*>(n->next)) {
					add_to_bucket(dst,
							new (my_allocator) node(n->item.first,
									n->item.second));
...
template<typename Key, typename T, typename HashCompare, typename A>
template<typename I>
void concurrent_hash_map<Key, T, HashCompare, A>::internal_copy(I first,
		I last) {
	hashcode_t m = my_mask;
	for (; first != last; ++first) {
...
template<typename Key, typename T, typename HashCompare, typename A1,
		typename A2>
