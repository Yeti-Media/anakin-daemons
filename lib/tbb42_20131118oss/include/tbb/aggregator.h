...
public:
	enum aggregator_operation_status {
		agg_waiting = 0, agg_finished
	};
	aggregator_operation() :
			status(agg_waiting), my_next(NULL) {
	}
...
public:
	basic_operation(const Body& b) :
			basic_operation_base(), my_body(b) {
	}
};
...
protected:
	...
	void execute_impl(aggregator_operation& op) {
		...
		if (!res) { // first in the list; handle the operations
			...
			spin_wait_while_eq(op.status,
					uintptr_t(aggregator_operation::agg_waiting));
