...
protected:
	...
	static bool is_first_element_in_segment(size_type element_index) {
		...
		__TBB_ASSERT(element_index,
				"there should be no need to call " "is_first_element_in_segment for 0th element");
	...
	void __TBB_EXPORTED_METHOD internal_reserve(size_type n,
			size_type element_size, size_type max_size);
	size_type __TBB_EXPORTED_METHOD internal_capacity() const;
	void internal_grow(size_type start, size_type finish,
	...
	void* __TBB_EXPORTED_METHOD internal_push_back(size_type element_size,
			size_type& index);
	segment_index_t __TBB_EXPORTED_METHOD internal_clear(
	...
	void __TBB_EXPORTED_METHOD internal_copy(
			const concurrent_vector_base_v3& src, size_type element_size,
			internal_array_op2 copy);
	void __TBB_EXPORTED_METHOD internal_assign(
			const concurrent_vector_base_v3& src, size_type element_size,
	...
	void __TBB_EXPORTED_METHOD internal_resize(size_type n,
			size_type element_size, size_type max_size, const void *src,
...
public: // workaround for MSVC
	...
	vector_iterator(const Container& vector, size_t index, void *ptr = 0) :
			my_vector(const_cast<Container*>(&vector)), my_index(index), my_item(
					static_cast<Value*>(ptr)) {
	}
...
public:
	...
	vector_iterator(
			...
			my_vector(other.my_vector), my_index(other.my_index), my_item(
					other.my_item) {
	}
...
vector_iterator<Container, T> operator+(ptrdiff_t offset,
...
public:
	...
	//! Copying constructor
	concurrent_vector(const concurrent_vector& vector, const allocator_type& a =
			allocator_type()) :
			internal::allocator_base<T, A>(a), internal::concurrent_vector_base() {
		vector_allocator_ptr = &internal_allocator;
		__TBB_TRY {
	...
	concurrent_vector(size_type n, const_reference t, const allocator_type& a =
			allocator_type()) :
	...
	template<class I>
	concurrent_vector(I first, I last, const allocator_type &a =
			allocator_type()) :
			internal::allocator_base<T, A>(a) {
		vector_allocator_ptr = &internal_allocator;
		__TBB_TRY {
			internal_assign_range(first, last,
					static_cast<is_integer_tag<
							std::numeric_limits<I>::is_integer> *>(0));
		}__TBB_CATCH(...) {
			segment_t *table = my_segment;
			internal_free_segments(reinterpret_cast<void**>(table),
					internal_clear(&destroy_array), my_first_block);
	...
	iterator grow_to_at_least(size_type n) {
		size_type m = 0;
		if (n) {
			m = internal_grow_to_at_least_with_result(n, sizeof(T),
...
private:
	...
	static void *internal_allocator(internal::concurrent_vector_base_v3 &vb,
			size_t k) {
		return static_cast<concurrent_vector<T, A>&>(vb).my_allocator.allocate(
				k);
	}
	...
	void internal_assign_n(size_type n, const_pointer p) {
		internal_resize(n, sizeof(T), max_size(), static_cast<const void*>(p),
				&destroy_array, p ? &initialize_array_by : &initialize_array);
	}
...
template<typename T, class A>
void concurrent_vector<T, A>::internal_free_segments(void *table[],
		segment_index_t k, segment_index_t first_block) {
	// Free the arrays
	while (k > first_block) {
		--k;
...
T& concurrent_vector<T, A>::internal_subscript(size_type index) const {
	...
	__TBB_ASSERT(
			(segment_t*)my_segment != my_storage || k < pointers_per_short_table,
			"index is being allocated");
	// no need in __TBB_load_with_acquire since thread works in own space or gets 
	T* array = static_cast<T*>(tbb::internal::itt_hide_load_word(
			my_segment[k].array));
