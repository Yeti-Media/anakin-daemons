...
public:
	...
	public:
		...
		nodeptr_t atomic_set_next(nodeptr_t new_node, nodeptr_t current_node) {
			...
			nodeptr_t exchange_node =
					tbb::internal::as_atomic(my_next).compare_and_swap(new_node,
							current_node);
	...
	nodeptr_t try_insert(nodeptr_t previous, nodeptr_t new_node,
	...
	std::pair<iterator, bool> try_insert(raw_iterator it, raw_iterator next,
			const value_type &value, sokey_t order_key, size_type *new_count) {
		nodeptr_t pnode = create_node(order_key, value);
		nodeptr_t inserted_node = try_insert(it.get_node_ptr(), pnode,
				next.get_node_ptr());
	...
	raw_iterator insert_dummy(raw_iterator it, sokey_t order_key) {
		...
		for (;;) {
			...
			if (where == last || get_order_key(where) > order_key) {
				...
				nodeptr_t inserted_node = try_insert(it.get_node_ptr(),
...
protected:
	...
	concurrent_unordered_base(size_type n_of_buckets = initial_bucket_number,
			const hash_compare& hc = hash_compare(), const allocator_type& a =
					allocator_type()) :
			Traits(hc), my_solist(a), my_allocator(a), my_maximum_bucket_size(
					(float) initial_bucket_load) {
		if (n_of_buckets == 0)
			++n_of_buckets;
		my_number_of_buckets =
				1 << __TBB_Log2((uintptr_t )n_of_buckets * 2 - 1); // round up to power of 2
		internal_init();
	}
	...
	concurrent_unordered_base(const concurrent_unordered_base& right) :
			Traits(right.my_hash_compare), my_solist(right.get_allocator()), my_allocator(
					right.get_allocator()) {
		internal_init();
		internal_copy(right);
	}
	...
	concurrent_unordered_base& operator=(
			std::initializer_list<value_type> const& il) {
		this->clear();
...
public:
	...
	public:
		...
		const_range_type(const concurrent_unordered_base &a_table) :
				my_table(a_table), my_begin_node(a_table.my_solist.begin()), my_end_node(
						a_table.my_solist.end()) {
			set_midpoint();
		}
		...
		void set_midpoint() const {
			if (my_begin_node == my_end_node) // not divisible
				...
				size_t mid_bucket = __TBB_ReverseBits(
						begin_key + (end_key - begin_key) / 2)
	...
	size_type count(const key_type& key) const {
		if (allow_multimapping) {
			...
			size_type item_count = internal_distance(answer.first,
					answer.second);
			return item_count;
		} else {
			return const_cast<self_type*>(this)->internal_find(key) == end() ?
					0 : 1;
		}
	}
...
private:
	...
	// Hash APIs
	size_type internal_distance(const_iterator first,
	...
	std::pair<iterator, bool> internal_insert(const value_type& value) {
		...
		for (;;) {
			if (where == last || solist_t::get_order_key(where) > order_key) {
				...
				return std::pair<iterator, bool>(my_solist.get_iterator(where),
						false);
			}
	...
	void init_bucket(size_type bucket) {
		...
		raw_iterator dummy_node = my_solist.insert_dummy(parent,
				split_order_key_dummy(bucket));
...
//! Hash multiplier
static const size_t hash_multiplier = tbb::internal::select_size_t_constant<
