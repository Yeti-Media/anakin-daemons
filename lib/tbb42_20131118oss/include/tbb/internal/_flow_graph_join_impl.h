...
struct forwarding_base {
	forwarding_base(graph &g) :
			my_graph_ptr(&g), current_tag(NO_TAG) {
	}
	...
	virtual task * increment_tag_count(tag_value /*t*/, bool /*handle_task*/) {
		return NULL;
	}
...
struct join_helper {
	...
	static inline bool reserve(InputTuple &my_input, OutputTuple &out) {
		if (!tbb::flow::get < N - 1
				> (my_input).reserve(tbb::flow::get < N - 1 > (out)))
			return false;
	...
	static inline bool get_my_item(InputTuple &my_input, OutputTuple &out) {
		bool res = tbb::flow::get < N - 1
				> (my_input).get_item(tbb::flow::get < N - 1 > (out)); // may fail
	...
	static inline void set_tag_func(InputTuple &my_input,
			TagFuncTuple &my_tag_funcs) {
		tbb::flow::get < N - 1
				> (my_input).set_my_original_tag_func(
						tbb::flow::get < N - 1 > (my_tag_funcs));
		tbb::flow::get < N - 1
				> (my_input).set_my_tag_func(
						tbb::flow::get < N - 1
								> (my_input).my_original_func()->clone());
	...
	static inline void copy_tag_functors(TagFuncTuple1 &my_inputs,
			TagFuncTuple2 &other_inputs) {
		if (tbb::flow::get < N - 1 > (other_inputs).my_original_func()) {
			tbb::flow::get < N - 1
					> (my_inputs).set_my_tag_func(
							tbb::flow::get < N - 1
									> (other_inputs).my_original_func()->clone());
			tbb::flow::get < N - 1
					> (my_inputs).set_my_original_tag_func(
							tbb::flow::get < N - 1
									> (other_inputs).my_original_func()->clone());
		}
...
struct join_helper<1> {
	...
	template<typename InputTuple, typename OutputTuple>
	static inline bool reserve(InputTuple &my_input, OutputTuple &out) {
		return tbb::flow::get < 0
				> (my_input).reserve(tbb::flow::get < 0 > (out));
	}

	template<typename InputTuple, typename OutputTuple>
	static inline bool get_my_item(InputTuple &my_input, OutputTuple &out) {
		return tbb::flow::get < 0
				> (my_input).get_item(tbb::flow::get < 0 > (out));
	}
...
private:
	// ----------- Aggregator ------------
	enum op_type {
		reg_pred, rem_pred, res_item, rel_res, con_res
	};
	enum op_stat {
	...
	class reserving_port_operation: public aggregated_operation<
	...
	friend class internal::aggregating_functor<my_class,
			reserving_port_operation>;
...
private:
	enum op_type {
		get__item, res_port, try__put_task
	};
	enum op_stat {
		WAIT = 0, SUCCEEDED, FAILED
	};
	...
	class queueing_port_operation: public aggregated_operation<
	...
	public:
		...
		queueing_port_operation(const T* p, op_type t) :
				type(char(t)), my_arg(const_cast<T*>(p)), bypass_t(NULL) {
		}
	...
	void handle_operations(queueing_port_operation* op_list) {
		...
		while (op_list) {
			...
			case get__item:
				...
				break;
			case res_port:
				__TBB_ASSERT(this->item_valid(this->my_head),
						"No item to reset");
...
template<typename T>
class tag_matching_port: public receiver<T>, public tagged_buffer<tag_value, T,
		NO_TAG> {
public:
	typedef T input_type;
	typedef sender<T> predecessor_type;
...
private:
	...
	class tag_matching_port_operation: public aggregated_operation<
	...
	public:
		...
		// constructor with no parameter
		tag_matching_port_operation(op_type t) :
				type(char(t)) {
		}
	};
	...
	void handle_operations(tag_matching_port_operation* op_list) {
		...
		while (op_list) {
			...
			switch (current->type) {
			case try__put: {
				bool was_inserted = this->tagged_insert(current->my_tag_value,
						current->my_val);
				// return failure if a duplicate insertion occurs
				__TBB_store_with_release(current->status,
						was_inserted ? SUCCEEDED : FAILED);
			}
				break;
			case get__item:
				// use current_tag from FE for item
				if (!this->tagged_find(my_join->current_tag,
						*(current->my_arg))) {
					__TBB_ASSERT(false,
							"Failed to find item corresponding to current_tag.");
				}
				__TBB_store_with_release(current->status, SUCCEEDED);
				break;
			case res_port:
...
public:
	...
	tag_matching_port() :
			receiver<T>(), tagged_buffer<tag_value, T, NO_TAG>() {
		my_join = NULL;
		my_tag_func = NULL;
	...
	// copy constructor
	tag_matching_port(const tag_matching_port& /*other*/) :
			receiver<T>(), tagged_buffer<tag_value, T, NO_TAG>() {
		my_join = NULL;
	...
	~tag_matching_port() {
		if (my_tag_func)
			delete my_tag_func;
		if (my_original_tag_func)
			delete my_original_tag_func;
	}
	...
	my_tag_func_type *my_func() {
		return my_tag_func;
	}
	my_tag_func_type *my_original_func() {
		return my_original_tag_func;
	}
...
public:
	...
	join_node_FE(graph &g) :
			forwarding_base(g), my_node(NULL) {
		ports_with_no_inputs = N;
		join_helper<N>::set_join_node_pointer(my_inputs, this);
	}

	join_node_FE(const join_node_FE& other) :
			forwarding_base(*(other.forwarding_base::my_graph_ptr)), my_node(
					NULL) {
		ports_with_no_inputs = N;
		join_helper<N>::set_join_node_pointer(my_inputs, this);
	}

	void set_my_node(my_node_type *new_my_node) {
		my_node = new_my_node;
	}
	...
	task * decrement_port_count(bool handle_task) {
		if (ports_with_no_inputs.fetch_and_decrement() == 1) {
			...
			if (tp) {
				task *rtask =
						new (task::allocate_additional_child_of(*tp)) forward_task_bypass<
...
class join_node_FE<tag_matching, InputTuple, OutputTuple> : public forwarding_base,
		...
		public tagged_buffer<tag_value, size_t, NO_TAG>,
...
private:
	enum op_type {
		res_count, inc_count, may_succeed, try_make
	};
	enum op_stat {
		WAIT = 0, SUCCEEDED, FAILED
	};
	...
	class tag_matching_FE_operation: public aggregated_operation<
			tag_matching_FE_operation> {
	public:
		char type;
		union {
			tag_value my_val;
		...
		bool enqueue_task;
		// constructor for value parameter
		tag_matching_FE_operation(const tag_value& e, bool q_task, op_type t) :
				type(char(t)), my_val(e), bypass_t(NULL), enqueue_task(q_task) {
		}
		tag_matching_FE_operation(output_type *p, op_type t) :
				type(char(t)), my_output(p), bypass_t(NULL), enqueue_task(true) {
		}
		// constructor with no parameter
		tag_matching_FE_operation(op_type t) :
				type(char(t)), bypass_t(NULL), enqueue_task(true) {
		}
	};
		...
		while (op_list) {
			...
			case inc_count: {  // called from input ports
				...
				if (!(this->tagged_find_ref(t, p))) {
					...
					if (!(this->tagged_find_ref(t, p))) {
						__TBB_ASSERT(false,
								"should find tag after inserting it");
					}
				}
				if (++(*p) == size_t(N)) {
...
public:
	...
	join_node_FE(graph &g, FunctionTuple tag_funcs) :
			forwarding_base(g), my_node(NULL) {
		join_helper<N>::set_join_node_pointer(my_inputs, this);
	...
	join_node_FE(const join_node_FE& other) :
			forwarding_base(*(other.forwarding_base::my_graph_ptr)), my_tag_buffer(), output_buffer_type() {
		my_node = NULL;
		join_helper<N>::set_join_node_pointer(my_inputs, this);
		join_helper<N>::copy_tag_functors(my_inputs,
				const_cast<input_type &>(other.my_inputs));
		my_aggregator.initialize_handler(my_handler(this));
	}
...
//! join_node_base
template<graph_buffer_policy JP, typename InputTuple, typename OutputTuple>
class join_node_base: public graph_node, public join_node_FE<JP, InputTuple,
		OutputTuple>, public sender<OutputTuple> {
protected:
...
private:
	// ----------- Aggregator ------------
	enum op_type {
		reg_succ, rem_succ, try__get, do_fwrd, do_fwrd_bypass
	};
	enum op_stat {
		WAIT = 0, SUCCEEDED, FAILED
	};
	...
	class join_node_base_operation: public aggregated_operation<
	...
	public:
		...
		join_node_base_operation(const output_type& e, op_type t) :
				type(char(t)), my_arg(const_cast<output_type*>(&e)), bypass_t(
						NULL) {
		}
		join_node_base_operation(const successor_type &s, op_type t) :
				type(char(t)), my_succ(const_cast<successor_type *>(&s)), bypass_t(
						NULL) {
		}
...
public:
	...
	join_node_base(const join_node_base& other) :
			graph_node(other.graph_node::my_graph), input_ports_type(other), sender<
					OutputTuple>(), forwarder_busy(false), my_successors() {
		my_successors.set_owner(this);
		input_ports_type::set_my_node(this);
		my_aggregator.initialize_handler(my_handler(this));
	}
...
template<int N, template<class > class PT, typename OutputTuple,
		graph_buffer_policy JP>
struct join_base {
	typedef typename internal::join_node_base<JP,
			typename wrap_tuple_elements<N, PT, OutputTuple>::type, OutputTuple> type;
};
...
class unfolded_join_node<2, tag_matching_port, OutputTuple, tag_matching> : public join_base<
		2, tag_matching_port, OutputTuple, tag_matching>::type {
...
public:
	...
	unfolded_join_node(graph &g, B0 b0, B1 b1) :
			base_type(g,
					func_initializer_type(
							new internal::function_body_leaf<T0, tag_value, B0>(
									b0),
							new internal::function_body_leaf<T1, tag_value, B1>(
									b1))) {
	}
	unfolded_join_node(const unfolded_join_node &other) :
			base_type(other) {
...
template<typename OutputTuple>
class unfolded_join_node<3, tag_matching_port, OutputTuple, tag_matching> : public join_base<
		3, tag_matching_port, OutputTuple, tag_matching>::type {
	typedef typename tbb::flow::tuple_element<0, OutputTuple>::type T0;
	typedef typename tbb::flow::tuple_element<1, OutputTuple>::type T1;
...
public:
	...
	unfolded_join_node(graph &g, B0 b0, B1 b1, B2 b2) :
			base_type(g,
					func_initializer_type(
							new internal::function_body_leaf<T0, tag_value, B0>(
									b0),
							new internal::function_body_leaf<T1, tag_value, B1>(
									b1),
							new internal::function_body_leaf<T2, tag_value, B2>(
									b2))) {
	}
	unfolded_join_node(const unfolded_join_node &other) :
			base_type(other) {
...
template<typename OutputTuple>
class unfolded_join_node<4, tag_matching_port, OutputTuple, tag_matching> : public join_base<
		4, tag_matching_port, OutputTuple, tag_matching>::type {
	typedef typename tbb::flow::tuple_element<0, OutputTuple>::type T0;
	typedef typename tbb::flow::tuple_element<1, OutputTuple>::type T1;
	typedef typename tbb::flow::tuple_element<2, OutputTuple>::type T2;
...
public:
	...
	unfolded_join_node(graph &g, B0 b0, B1 b1, B2 b2, B3 b3) :
			base_type(g,
					func_initializer_type(
							new internal::function_body_leaf<T0, tag_value, B0>(
									b0),
							...
							new internal::function_body_leaf<T2, tag_value, B2>(
									b2),
							new internal::function_body_leaf<T3, tag_value, B3>(
									b3))) {
	}
	unfolded_join_node(const unfolded_join_node &other) :
			base_type(other) {
...
template<typename OutputTuple>
class unfolded_join_node<5, tag_matching_port, OutputTuple, tag_matching> : public join_base<
		5, tag_matching_port, OutputTuple, tag_matching>::type {
	typedef typename tbb::flow::tuple_element<0, OutputTuple>::type T0;
	typedef typename tbb::flow::tuple_element<1, OutputTuple>::type T1;
	typedef typename tbb::flow::tuple_element<2, OutputTuple>::type T2;
	typedef typename tbb::flow::tuple_element<3, OutputTuple>::type T3;
...
public:
	...
	unfolded_join_node(graph &g, B0 b0, B1 b1, B2 b2, B3 b3, B4 b4) :
			base_type(g,
					func_initializer_type(
							new internal::function_body_leaf<T0, tag_value, B0>(
									b0),
							...
							new internal::function_body_leaf<T3, tag_value, B3>(
									b3),
							new internal::function_body_leaf<T4, tag_value, B4>(
									b4))) {
	}
	unfolded_join_node(const unfolded_join_node &other) :
			base_type(other) {
...
typename tbb::flow::tuple_element<N, typename JNT::input_ports_type>::type &input_port(
		JNT &jn) {
