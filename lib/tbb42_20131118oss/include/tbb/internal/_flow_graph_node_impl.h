...
class function_input_base: public receiver<Input>, tbb::internal::no_assign {
	...
	enum op_type {
		reg_pred, rem_pred, app_body, try_fwd, tryput_bypass, app_body_bypass
	};
...
public:
	...
	function_input_base(graph &g, size_t max_concurrency,
			...
			my_graph(g), my_max_concurrency(max_concurrency), my_concurrency(0), my_queue(
					q), forwarder_busy(false) {
	...
	function_input_base(const function_input_base& src,
			function_input_queue<input_type, A> *q = NULL) :
			receiver<Input>(), tbb::internal::no_assign(), my_graph(
					src.my_graph), my_max_concurrency(src.my_max_concurrency), my_concurrency(
					0), my_queue(q), forwarder_busy(false) {
		my_predecessors.set_owner(this);
		my_aggregator.initialize_handler(my_handler(this));
	}
...
private:
	...
	task * apply_body_bypass(input_type &i) {
		task * new_task = static_cast<ImplType *>(this)->apply_body_impl_bypass(
				i);
		if (my_max_concurrency != 0) {
	...
	inline task * create_body_task(const input_type &input) {
		...
		return (tp) ?
				new (task::allocate_additional_child_of(*tp)) apply_body_task_bypass<
						my_class, input_type>(*this, input) :
				NULL;
	}
...
class function_input: public function_input_base<Input, A,
		function_input<Input, Output, A> > {
public:
	typedef Input input_type;
	...
	function_input(graph &g, size_t max_concurrency, Body& body,
			function_input_queue<input_type, A> *q = NULL) :
			base_type(g, max_concurrency, q), my_body(
					new internal::function_body_leaf<input_type, output_type,
							Body>(body)) {
	}

	//! Copy constructor
	function_input(const function_input& src, input_queue_type *q = NULL) :
			base_type(src, q), my_body(src.my_body->clone()) {
	}
	...
	Body copy_function_object() {
		internal::function_body<input_type, output_type> &body_ref =
				*this->my_body;
		return dynamic_cast<internal::function_body_leaf<input_type,
				output_type, Body> &>(body_ref).get_body();
	}
...
class multifunction_input: public function_input_base<Input, A,
		multifunction_input<Input, OutputPortSet, A> > {
public:
	typedef Input input_type;
	...
	//! Copy constructor
	multifunction_input(const multifunction_input& src, input_queue_type *q =
			NULL) :
			base_type(src, q), my_body(src.my_body->clone()) {
	}
	...
	template<typename Body>
	Body copy_function_object() {
		internal::multifunction_body<input_type, output_ports_type> &body_ref =
				*this->my_body;
		return dynamic_cast<internal::multifunction_body_leaf<input_type,
				output_ports_type, Body> &>(body_ref).get_body();
	}
...
typename tbb::flow::tuple_element<N, typename MOP::output_ports_type>::type &output_port(
		MOP &op) {
...
public:
	...
	continue_input(graph &g, int number_of_predecessors, Body& body) :
			continue_receiver(number_of_predecessors), my_graph_ptr(&g), my_body(
					new internal::function_body_leaf<input_type, output_type,
							Body>(body)) {
	}

	continue_input(const continue_input& src) :
			continue_receiver(src), my_graph_ptr(src.my_graph_ptr), my_body(
					src.my_body->clone()) {
	}
...
protected:
	...
	task *execute() {
		...
		return (tp) ?
				new (task::allocate_additional_child_of(*tp)) apply_body_task_bypass<
						continue_input<Output>, continue_msg>(*this,
						continue_msg()) :
				NULL;
	}
...
protected:
	...
	broadcast_cache<output_type> &successors() {
