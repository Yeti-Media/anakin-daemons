...
protected:
	...
	void fetch_front(T &v) {
		__TBB_ASSERT(item_valid(my_head), "front not valid");
		v = item(my_head).first;
	}
	void fetch_back(T &v) {
		__TBB_ASSERT(item_valid(my_tail - 1), "back not valid");
		v = item(my_tail - 1).first;
	}

	void invalidate(size_type i) {
		__TBB_ASSERT(item_valid(i), "Item not valid");
		item(i).second = false;
	}
	void validate(size_type i) {
		__TBB_ASSERT(!item_valid(i), "Item already valid");
		item(i).second = true;
	}
	...
	void invalidate_back() {
		invalidate(my_tail - 1);
	}

	size_type size() {
		return my_tail - my_head;
	}
	size_type capacity() {
...
public:
	reservable_item_buffer() :
			item_buffer<T, A>(), my_reserved(false) {
	}
	void reset() {
		my_reserved = false;
...
protected:
	...
	bool reserve_front(T &v) {
		if (my_reserved || !item_valid(my_head))
			return false;
		my_reserved = true;
		// reserving the head
		fetch_front(v);
