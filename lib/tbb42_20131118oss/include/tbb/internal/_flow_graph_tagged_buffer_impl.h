...
struct buffer_element {
	...
	buffer_element *next;
	buffer_element() :
			t(NoTagMark), next(NULL) {
	}
};

template<typename TagType, typename ValueType, size_t NoTagMark = 0,
		typename Allocator = tbb::cache_aligned_allocator<
				buffer_element<TagType, ValueType, NoTagMark> > >
class tagged_buffer {
public:
...
private:
	...
	void set_up_free_list(element_type **p_free_list, list_array_type *la,
			size_t sz) {
	...
	void grow_array() {
		...
		for (size_t i = 0; i < my_size; ++i)
			new_array[i] = NULL;
		list_array_type *new_list_array = new list_array_type(old_size,
				element_type(), Allocator());
	...
	void internal_tagged_insert(element_type **ar, size_t sz, TagType t,
			value_type v) {
		size_t l_mask = sz - 1;
		size_t h = hash(t) & l_mask;
		__TBB_ASSERT(free_list, "Error: free list not set up.");
		element_type* my_elem = free_list;
	...
	void internal_initialize_buffer() {
		...
		for (size_t i = 0; i < my_size; ++i)
			array[i] = NULL;
		lists = new list_array_type(INITIAL_SIZE / 2, element_type(),
				Allocator());
...
public:
	tagged_buffer() :
			my_size(INITIAL_SIZE), nelements(0) {
