...
// needed for resetting decrementer
...
public:
	...
	//! Default constructor
	graph_iterator() :
			my_graph(NULL), current_node(NULL) {
	}
...
public:
	...
	explicit graph() :
			my_nodes(NULL), my_nodes_last(NULL) {
		own_context = true;
		cancelled = false;
	...
	void run(Receiver &r, Body body) {
		FLOW_SPAWN(
				(*new (task::allocate_additional_child_of(*my_root_task)) run_and_put_task<
						Receiver, Body>(r, body)));
	}
	...
	void run(Body body) {
		FLOW_SPAWN(
				*new (task::allocate_additional_child_of(*my_root_task)) run_task<
						Body>(body));
	}
...
public:
	...
	source_node(graph &g, Body body, bool is_active = true) :
			graph_node(g), my_active(is_active), init_my_active(is_active), my_body(
					new internal::source_body_leaf<output_type, Body>(body)), my_reserved(
					false), my_has_cached_item(false) {
		my_successors.set_owner(this);
		tbb::internal::fgt_node_with_body(tbb::internal::FLOW_SOURCE_NODE,
				&this->my_graph, static_cast<sender<output_type> *>(this),
				this->my_body);
	}

	//! Copy constructor
	source_node(const source_node& src) :
			graph_node(src.my_graph), sender<Output>(), my_active(
					src.init_my_active), init_my_active(src.init_my_active), my_body(
					src.my_body->clone()), my_reserved(false), my_has_cached_item(
					false) {
		my_successors.set_owner(this);
		tbb::internal::fgt_node_with_body(tbb::internal::FLOW_SOURCE_NODE,
				&this->my_graph, static_cast<sender<output_type> *>(this),
...
private:
	...
	void spawn_put() {
		...
		if (tp) {
			FLOW_SPAWN(
					(*new (task::allocate_additional_child_of(*tp)) internal::source_task_bypass<
							source_node<output_type> >(*this)));
		}
	}
...
template<typename Input, typename Output = continue_msg, graph_buffer_policy =
		queueing, typename Allocator = cache_aligned_allocator<Input> >
class function_node: public graph_node, public internal::function_input<Input,
		Output, Allocator>, public internal::function_output<Output> {
protected:
	using graph_node::my_graph;
public:
	...
	function_node(graph &g, size_t concurrency, Body body) :
			graph_node(g), internal::function_input<input_type, output_type,
					Allocator>(g, concurrency, body) {
		tbb::internal::fgt_node_with_body(tbb::internal::FLOW_FUNCTION_NODE,
	...
	function_node(const function_node& src) :
			graph_node(src.my_graph), internal::function_input<input_type,
					output_type, Allocator>(src), fOutput_type() {
		tbb::internal::fgt_node_with_body(tbb::internal::FLOW_FUNCTION_NODE,
...
protected:
	...
	/*override*/
	void reset() {
		fInput_type::reset_function_input();
	}
...
class function_node<Input, Output, queueing, Allocator> : public graph_node,
...
public:
	...
	//! Copy constructor
	function_node(const function_node& src) :
			graph_node(src.graph_node::my_graph), fInput_type(src,
					new queue_type()), fOutput_type() {
		tbb::internal::fgt_node_with_body(tbb::internal::FLOW_FUNCTION_NODE,
				&this->graph_node::my_graph,
				static_cast<receiver<input_type> *>(this),
				static_cast<sender<output_type> *>(this), this->my_body);
...
public:
	typedef Input input_type;
	typedef typename internal::wrap_tuple_elements<N,
			internal::multifunction_output, Output>::type output_ports_type;
private:
	typedef typename internal::multifunction_input<input_type,
			output_ports_type, Allocator> base_type;
...
public:
	...
	multifunction_node(graph &g, size_t concurrency, Body body) :
		...
		tbb::internal::fgt_multioutput_node_with_body<Output, N>(
...
template<typename Input, typename Output, typename Allocator>
class multifunction_node<Input, Output, queueing, Allocator> : public graph_node,
		public internal::multifunction_input<Input,
				typename internal::wrap_tuple_elements<
						tbb::flow::tuple_size<Output>::value,
						internal::multifunction_output, Output>::type, Allocator> {
protected:
	using graph_node::my_graph;
	static const int N = tbb::flow::tuple_size<Output>::value;
public:
...
template<typename TupleType, typename Allocator = cache_aligned_allocator<
		TupleType> >
class split_node: public multifunction_node<TupleType, TupleType, rejecting,
		Allocator> {
...
public:
	...
	split_node(graph &g) :
		...
		tbb::internal::fgt_multioutput_node<TupleType, N>(
				tbb::internal::FLOW_SPLIT_NODE, &this->graph_node::my_graph,
				static_cast<receiver<input_type> *>(this),
				this->output_ports());
	}
...
public:
	...
	continue_node(graph &g, Body body) :
		...
		tbb::internal::fgt_node_with_body(tbb::internal::FLOW_CONTINUE_NODE,
				&this->my_graph, static_cast<receiver<input_type> *>(this),
				static_cast<sender<output_type> *>(this), this->my_body);
	}
	...
	continue_node(graph &g, int number_of_predecessors, Body body) :
			graph_node(g), internal::continue_input<output_type>(g,
					number_of_predecessors, body) {
		tbb::internal::fgt_node_with_body(tbb::internal::FLOW_CONTINUE_NODE,
				&this->my_graph, static_cast<receiver<input_type> *>(this),
				static_cast<sender<output_type> *>(this), this->my_body);
	}
	...
	continue_node(const continue_node& src) :
			graph_node(src.graph_node::my_graph), internal::continue_input<
					output_type>(src), internal::function_output<Output>() {
		tbb::internal::fgt_node_with_body(tbb::internal::FLOW_CONTINUE_NODE,
				&this->my_graph, static_cast<receiver<input_type> *>(this),
				static_cast<sender<output_type> *>(this), this->my_body);
	}
...
protected:
	...
	/*override*/
	void reset() {
		internal::continue_input<Output>::reset_receiver();
	}
...
public:
	...
	overwrite_node(graph &g) :
		...
		my_successors.set_owner(this);
		tbb::internal::fgt_node(tbb::internal::FLOW_OVERWRITE_NODE,
				&this->my_graph, static_cast<receiver<input_type> *>(this),
	...
	overwrite_node(const overwrite_node& src) :
			graph_node(src.my_graph), receiver<T>(), sender<T>(), my_buffer_is_valid(
					false) {
		my_successors.set_owner(this);
		tbb::internal::fgt_node(tbb::internal::FLOW_OVERWRITE_NODE,
				&this->my_graph, static_cast<receiver<input_type> *>(this),
...
public:
	...
	write_once_node(graph& g) :
			overwrite_node<T>(g) {
		tbb::internal::fgt_node(tbb::internal::FLOW_WRITE_ONCE_NODE,
				&(this->my_graph), static_cast<receiver<input_type> *>(this),
				static_cast<sender<output_type> *>(this));
	}
...
public:
	...
	broadcast_node(graph& g) :
			graph_node(g) {
		my_successors.set_owner(this);
		tbb::internal::fgt_node(tbb::internal::FLOW_BROADCAST_NODE,
				&this->my_graph, static_cast<receiver<input_type> *>(this),
				static_cast<sender<output_type> *>(this));
	}
...
class buffer_node: public graph_node,
		public reservable_item_buffer<T, A>,
		public receiver<T>,
		public sender<T> {
protected:
	using graph_node::my_graph;
...
protected:
	...
	class buffer_operation: public internal::aggregated_operation<
			buffer_operation> {
	public:
		char type;
		T *elem;
		...
		buffer_operation(const T& e, op_type t) :
				type(char(t)), elem(const_cast<T*>(&e)), ltask(NULL), r(NULL) {
		}
		buffer_operation(op_type t) :
				type(char(t)), ltask(NULL), r(NULL) {
		}
	};
	...
	virtual void handle_operations(buffer_operation *op_list) {
		...
		if (try_forwarding && !forwarder_busy) {
			...
			if (tp) {
				task *new_task =
						new (task::allocate_additional_child_of(*tp)) internal::forward_task_bypass<
	...
	virtual void internal_forward_task(buffer_operation *op) {
		...
		while (counter > 0 && !this->buffer_empty()
				&& this->item_valid(this->my_tail - 1)) {
...
public:
	//! Constructor
	buffer_node(graph &g) :
			graph_node(g), reservable_item_buffer<T>(), forwarder_busy(false) {
		my_successors.set_owner(this);
		my_aggregator.initialize_handler(my_handler(this));
		tbb::internal::fgt_node(tbb::internal::FLOW_BUFFER_NODE,
				&this->my_graph, static_cast<receiver<input_type> *>(this),
				static_cast<sender<output_type> *>(this));
	}
	...
	buffer_node(const buffer_node& src) :
			graph_node(src.my_graph), reservable_item_buffer<T>(), receiver<T>(), sender<
					T>() {
		forwarder_busy = false;
		my_successors.set_owner(this);
		my_aggregator.initialize_handler(my_handler(this));
...
public:
	...
	sequencer_node(graph &g, const Sequencer& s) :
			queue_node<T, A>(g), my_sequencer(
					new internal::function_body_leaf<T, size_t, Sequencer>(s)) {
		tbb::internal::fgt_node(tbb::internal::FLOW_SEQUENCER_NODE,
				&(this->my_graph), static_cast<receiver<input_type> *>(this),
				static_cast<sender<output_type> *>(this));
	}
...
private:
	/* override */
	void internal_push(sequencer_operation *op) {
...
protected:
	...
	void handle_operations(prio_operation *op_list) {
		...
		while (op_list) {
			...
			op_list = op_list->next;
			switch (tmp->type) {
			case buffer_node<T, A>::reg_succ:
				this->internal_reg_succ(tmp);
				try_forwarding = true;
				break;
			case buffer_node<T, A>::rem_succ:
				this->internal_rem_succ(tmp);
				break;
			case buffer_node<T, A>::put_item:
				internal_push(tmp);
				try_forwarding = true;
				break;
			case buffer_node<T, A>::try_fwd_task:
				internal_forward_task(tmp);
				break;
			case buffer_node<T, A>::rel_res:
				internal_release(tmp);
				try_forwarding = true;
				break;
			case buffer_node<T, A>::con_res:
				internal_consume(tmp);
				try_forwarding = true;
				break;
			case buffer_node<T, A>::req_item:
				internal_pop(tmp);
				break;
			case buffer_node<T, A>::res_item:
				internal_reserve(tmp);
				break;
			}
		}
							...
							task *rtask =
									new (
											task::allocate_additional_child_of(
													*tp)) internal::forward_task_bypass<
											limiter_node<T> >(*this);
							FLOW_SPAWN(*rtask);
						}
					}
...
public:
	...
	limiter_node(graph &g, size_t threshold, int num_decrement_predecessors = 0) :
			graph_node(g), my_threshold(threshold), my_count(0), my_tries(0), init_decrement_predecessors(
					num_decrement_predecessors), decrement(
					num_decrement_predecessors) {
		my_predecessors.set_owner(this);
		my_successors.set_owner(this);
		decrement.set_owner(this);
		tbb::internal::fgt_node(tbb::internal::FLOW_LIMITER_NODE,
				&this->my_graph, static_cast<receiver<input_type> *>(this),
	...
	limiter_node(const limiter_node& src) :
			graph_node(src.my_graph), receiver<T>(), sender<T>(), my_threshold(
					src.my_threshold), my_count(0), my_tries(0), init_decrement_predecessors(
					src.init_decrement_predecessors), decrement(
					src.init_decrement_predecessors) {
		my_predecessors.set_owner(this);
		my_successors.set_owner(this);
		decrement.set_owner(this);
		tbb::internal::fgt_node(tbb::internal::FLOW_LIMITER_NODE,
...
class join_node<OutputTuple, reserving> : public internal::unfolded_join_node<
		tbb::flow::tuple_size<OutputTuple>::value, reserving_port, OutputTuple,
		reserving> {
private:
	static const int N = tbb::flow::tuple_size<OutputTuple>::value;
	typedef typename internal::unfolded_join_node<N, reserving_port,
			OutputTuple, reserving> unfolded_type;
public:
	typedef OutputTuple output_type;
	typedef typename unfolded_type::input_ports_type input_ports_type;
	join_node(graph &g) :
			unfolded_type(g) {
		tbb::internal::fgt_multiinput_node<OutputTuple, N>(
				tbb::internal::FLOW_JOIN_NODE_RESERVING, &this->my_graph,
...
class join_node<OutputTuple, queueing> : public internal::unfolded_join_node<
		tbb::flow::tuple_size<OutputTuple>::value, queueing_port, OutputTuple,
		queueing> {
private:
	static const int N = tbb::flow::tuple_size<OutputTuple>::value;
	typedef typename internal::unfolded_join_node<N, queueing_port, OutputTuple,
			queueing> unfolded_type;
public:
	typedef OutputTuple output_type;
	typedef typename unfolded_type::input_ports_type input_ports_type;
	join_node(graph &g) :
			unfolded_type(g) {
		tbb::internal::fgt_multiinput_node<OutputTuple, N>(
				tbb::internal::FLOW_JOIN_NODE_QUEUEING, &this->my_graph,
				this->input_ports(), static_cast<sender<output_type> *>(this));
	}
	join_node(const join_node &other) :
			unfolded_type(other) {
...
template<typename OutputTuple>
class join_node<OutputTuple, tag_matching> : public internal::unfolded_join_node<
		tbb::flow::tuple_size<OutputTuple>::value, tag_matching_port,
		OutputTuple, tag_matching> {
private:
	static const int N = tbb::flow::tuple_size<OutputTuple>::value;
	typedef typename internal::unfolded_join_node<N, tag_matching_port,
			OutputTuple, tag_matching> unfolded_type;
public:
	typedef OutputTuple output_type;
	typedef typename unfolded_type::input_ports_type input_ports_type;
	...
	join_node(graph &g, __TBB_B0 b0, __TBB_B1 b1) :
		...
		tbb::internal::fgt_multiinput_node<OutputTuple, N>(
				tbb::internal::FLOW_JOIN_NODE_TAG_MATCHING, &this->my_graph,
				this->input_ports(), static_cast<sender<output_type> *>(this));
	}
