...
class finish_reduce: public flag_task {
	...
	finish_reduce(reduction_context context_) :
			...
			my_context(context_), my_body(NULL) {
	}
	~finish_reduce() {
		if (has_right_zombie)
...
public:
	...
	start_reduce(const Range& range, Body* body, Partitioner& partitioner) :
			my_body(body), my_range(range), my_partition(partitioner), my_context(
					root_task) {
	}
	//! Splitting constructor used to generate children.
	/** parent_ becomes left child.  Newly constructed object is right child. */
	start_reduce(start_reduce& parent_, split) :
			my_body(parent_.my_body), my_range(parent_.my_range, split()), my_partition(
					parent_.my_partition, split()), my_context(right_child) {
	...
	start_reduce(start_reduce& parent_, const Range& r, depth_t d) :
			my_body(parent_.my_body), my_range(r), my_partition(
					parent_.my_partition, split()), my_context(right_child) {
		my_partition.set_affinity(*this);
	...
	static void run(const Range& range, Body& body, Partitioner& partitioner) {
		if (!range.empty()) {
			...
			task::spawn_root_and_wait(
					*new (task::allocate_root(context)) start_reduce(range,
							&body, partitioner));
...
task* start_reduce<Range, Body, Partitioner>::execute() {
	...
	if (my_context == right_child) {
		...
		if (!itt_load_word_with_acquire(parent_ptr->my_body)) { // TODO: replace by is_stolen_task() or by parent_ptr->ref_count() == 2???
			my_body = new (parent_ptr->zombie_space.begin()) Body(*my_body,
					split());
...
class start_deterministic_reduce: public task {
	...
	start_deterministic_reduce(const Range& range, Body& body) :
			my_body(body), my_range(range) {
	}
	//! Splitting constructor used to generate children.
	/** parent_ becomes left child.  Newly constructed object is right child. */
	start_deterministic_reduce(start_deterministic_reduce& parent_,
			finish_type& c) :
			my_body(c.my_right_body), my_range(parent_.my_range, split()) {
	}
...
public:
	static void run(const Range& range, Body& body) {
		if (!range.empty()) {
			...
			task_group_context context;
			task::spawn_root_and_wait(
					*new (task::allocate_root(context)) start_deterministic_reduce(
							range, body));
#endif /* __TBB_TASK_GROUP_CONTEXT && !TBB_JOIN_OUTER_TASK_GROUP */
		}
	}
...
task* start_deterministic_reduce<Range, Body>::execute() {
	if (!my_range.is_divisible()) {
		...
		c.set_ref_count(2);
		start_deterministic_reduce& b =
				*new (c.allocate_child()) start_deterministic_reduce(*this, c);
		task::spawn(b);
		return this;
	}
}
...
public:
	lambda_reduce_body(const Value& identity, const RealBody& body,
			const Reduction& reduction) :
			identity_element(identity), my_real_body(body), my_reduction(
					reduction), my_value(identity) {
	}
	lambda_reduce_body(const lambda_reduce_body& other) :
			identity_element(other.identity_element), my_real_body(
					other.my_real_body), my_reduction(other.my_reduction), my_value(
					other.my_value) {
	}
	lambda_reduce_body(lambda_reduce_body& other, tbb::split) :
			identity_element(other.identity_element), my_real_body(
					other.my_real_body), my_reduction(other.my_reduction), my_value(
					other.identity_element) {
	}
...
void parallel_reduce(const Range& range, Body& body,
	...
	internal::start_reduce<Range, Body, affinity_partitioner>::run(range, body,
			partitioner);
}
...
Value parallel_deterministic_reduce(const Range& range, const Value& identity,
	...
	internal::lambda_reduce_body<Range, Value, RealBody, Reduction> body(
			identity, real_body, reduction);
	internal::start_deterministic_reduce<Range,
			internal::lambda_reduce_body<Range, Value, RealBody, Reduction> >::run(
			range, body);
	return body.result();
}
