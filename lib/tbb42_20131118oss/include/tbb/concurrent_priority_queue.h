...
public:
	...
	explicit concurrent_priority_queue(const allocator_type& a =
			allocator_type()) :
	...
	concurrent_priority_queue(std::initializer_list<T> const& init_list,
			const allocator_type &a = allocator_type()) :
	...
	explicit concurrent_priority_queue(const concurrent_priority_queue& src) :
			mark(src.mark), my_size(src.my_size), data(src.data.begin(),
					src.data.end(), src.data.get_allocator()) {
		my_aggregator.initialize_handler(my_functor_t(this));
		heapify();
	}
	...
	concurrent_priority_queue& operator=(const concurrent_priority_queue& src) {
		if (this != &src) {
			std::vector<value_type, allocator_type>(src.data.begin(),
					src.data.end(), src.data.get_allocator()).swap(data);
			mark = src.mark;
			my_size = src.my_size;
		}
		return *this;
	...
	void assign(InputIterator begin, InputIterator end) {
		std::vector<value_type, allocator_type>(begin, end,
				data.get_allocator()).swap(data);
		mark = 0;
		my_size = data.size();
		heapify();
	}
...
private:
	...
	char padding1[NFS_MaxLineSize
			- sizeof(aggregator<my_functor_t, cpq_operation> )];
	//! The point at which unsorted elements begin
	size_type mark;__TBB_atomic
	size_type my_size;
	Compare compare;
	//! Padding added to avoid false sharing
