...
/** The "posix_memalign" analogue.
 @ingroup memory_allocation */
int __TBB_EXPORTED_FUNC scalable_posix_memalign(void** memptr, size_t alignment,
		size_t size);
...
/** The "_aligned_realloc" analogue.
 @ingroup memory_allocation */
void * __TBB_EXPORTED_FUNC scalable_aligned_realloc(void* ptr, size_t size,
		size_t alignment);
...
struct MemPoolPolicy {
	...
	MemPoolPolicy(rawAllocType pAlloc_, rawFreeType pFree_,
			...
			pAlloc(pAlloc_), pFree(pFree_), granularity(granularity_), version(
					TBBMALLOC_POOL_VERSION), fixedPool(fixedPool_), keepAllMemory(
					keepAllMemory_), reserved(0) {
	}
};
...
void *pool_aligned_realloc(MemoryPool* mPool, void *ptr, size_t size,
		size_t alignment);
...
public:
	...
	template<typename U> scalable_allocator(
