...
template<size_t S>
struct atomic_rep;
// Primary template declared, but never defined.
...
__TBB_DECL_ATOMIC_LOAD_STORE_PRIMITIVES(full_fence);__TBB_DECL_ATOMIC_LOAD_STORE_PRIMITIVES(acquire);
__TBB_DECL_ATOMIC_LOAD_STORE_PRIMITIVES(release);__TBB_DECL_ATOMIC_LOAD_STORE_PRIMITIVES(relaxed);
...
#if !__SUNPRO_CC
		return *(typename converter<value_t>::bits_type*) ptr_converter<value_t*>(
				&value).bits;
#else
...
public:
	...
	value_type compare_and_swap(value_type value, value_type comparand) {
		return to_value<value_type>(
				internal::atomic_traits<sizeof(value_type), M>::compare_and_swap(
						&my_storage.my_value, to_bits(value),
						to_bits(comparand)));
	}
...
template<typename T> struct atomic<T*> : internal::atomic_impl_with_arithmetic<
		T*, ptrdiff_t, T> {
#if __TBB_ATOMIC_CTORS
	atomic() = default;
	constexpr atomic(T* arg) :
...
void store(atomic<T>& a, T value) {
	return a.template store<M>(value);
}
