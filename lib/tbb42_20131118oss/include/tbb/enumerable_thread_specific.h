...
public:
	...
	struct slot {
		...
		bool claim(key_type k) {
			...
			return tbb::internal::punned_cast<tbb::atomic<key_type>*>(&key)->compare_and_swap(
					k, 0) == 0;
		}
	};
#if __TBB_PROTECTED_NESTED_CLASS_BROKEN
protected:
	...
	array* allocate(size_t lg_size) {
		size_t n = 1 << lg_size;
		array* a = static_cast<array*>(create_array(
				sizeof(array) + n * sizeof(slot)));
		a->lg_size = lg_size;
	...
	static size_t hash(key_type k) {
		...
		return uintptr_t(k)
				* tbb::internal::select_size_t_constant<0x9E3779B9,
						0x9E3779B97F4A7C15ULL>::value;
	}
...
public:
	...
	enumerable_thread_specific_iterator(
			...
			my_container(other.my_container), my_index(other.my_index), my_value(
					const_cast<Value *>(other.my_value)) {
	}
	...
	Value& operator*() const {
		...
		if (!value) {
			value = my_value =
					reinterpret_cast<Value *>(&(*my_container)[my_index].value);
		}
		__TBB_ASSERT(
				value==reinterpret_cast<Value *>(&(*my_container)[my_index].value),
				"corrupt cache");
		return *value;
	}
...
enumerable_thread_specific_iterator<Container, T> operator+(ptrdiff_t offset,
	...
	return enumerable_thread_specific_iterator<Container, T>(v.my_container,
			v.my_index + offset);
}
...
public:
	...
	segmented_iterator(const SegmentedContainer& _segmented_container) :
			my_segcont(const_cast<SegmentedContainer*>(&_segmented_container)), outer_iter(
					my_segcont->end()) {
	}
...
class callback_leaf: public callback_base<T>, Constructor {
	template<typename X> callback_leaf(const X& x) :
			Constructor(x) {
	}
...
struct ets_element {
	char value[
			ModularSize == 0 ?
					sizeof(U) :
					sizeof(U) + (tbb::internal::NFS_MaxLineSize - ModularSize)];
	void unconstruct() {
...
class enumerable_thread_specific: internal::ets_base<ETS_key_type> {
	...
	void unconstruct_locals() {
		for (typename internal_collection_type::iterator cvi =
				my_locals.begin(); cvi != my_locals.end(); ++cvi) {
			cvi->unconstruct();
		}
	}
...
public:
	...
	typedef typename internal::enumerable_thread_specific_iterator<
			internal_collection_type, value_type> iterator;
	typedef typename internal::enumerable_thread_specific_iterator<
			internal_collection_type, const value_type> const_iterator;
	...
	enumerable_thread_specific(Finit finit) :
			my_construct_callback(
					internal::callback_leaf<T,
							internal::construct_by_finit<T, Finit> >::make(
							finit)) {
	}
	...
	enumerable_thread_specific(const T& exemplar) :
			my_construct_callback(
					internal::callback_leaf<T,
							internal::construct_by_exemplar<T> >::make(
							exemplar)) {
	}
...
public:
	...
	enumerable_thread_specific(
			const enumerable_thread_specific<U, Alloc, Cachetype>& other) :
...
template<typename U, typename A2, ets_key_usage_type C2>
void enumerable_thread_specific<T, Allocator, ETS_key_type>::internal_copy(
		const enumerable_thread_specific<U, A2, C2>& other) {
...
public:
	...
	flattened2d(const Container &c) :
			my_container(const_cast<Container*>(&c)), my_begin(c.begin()), my_end(
					c.end()) {
	}
