...
class parallel_do_operator_selector {
	...
	static void internal_call(const Body& obj, A1& arg1, A2&,
...
class do_iteration_task: public task {
	...
	/*override*/
	task* execute() {
		parallel_do_operator_selector<Body, Item>::call(*my_feeder.my_body,
				my_value, my_feeder);
		return NULL;
	}
...
class parallel_do_feeder_impl: public parallel_do_feeder<Item> {
	...
	void internal_add(const Item& item) {
		...
		iteration_type& t = *new (
				task::allocate_additional_child_of(*my_barrier)) iteration_type(
				item, *this);
...
class do_group_task_input: public task {
	...
	task* execute() {
		...
		size_t k = 0;
		for (;;) {
			t = new (allocate_child()) iteration_type(my_arg.begin() + k,
					my_feeder);
			if (++k == my_size)
				break;
			list.push_back(*t);
...
private:
	...
	task* run_for_input_iterator() {
		...
		block_type& t = *new (
				allocate_additional_child_of(*my_feeder.my_barrier)) block_type(
				my_feeder);
		size_t k = 0;
	...
	task* run_for_forward_iterator() {
		...
		return k == 0 ?
				NULL :
				new (allocate_additional_child_of(*my_feeder.my_barrier)) block_type(
						first, k, my_feeder);
	}
	...
	task* run_for_random_access_iterator() {
		...
		if (k > block_type::max_arg_size) {
			...
			do_task_iter& b = *new (c.allocate_child()) do_task_iter(middle,
					my_last, my_feeder);
...
void run_parallel_do(Iterator first, Iterator last, const Body& body
	...
	root_iteration_task &t =
			*new (feeder.my_barrier->allocate_child()) root_iteration_task(
					first, last, feeder);
...
void select_parallel_do(Iterator first, Iterator last, const Body& body,
	...
	run_parallel_do<Iterator, Body, typename strip<Item>::type>(first, last,
			body
#if __TBB_TASK_GROUP_CONTEXT
			, context
#endif // __TBB_TASK_GROUP_CONTEXT 
