...
__TBB_TASK_BASE_ACCESS:
	...
	static tbb::internal::allocate_additional_child_of_proxy allocate_additional_child_of(
			task& t) {
...
private:
	...
	enum version_traits_word_layout {
		traits_offset = 16, version_mask = 0xFFFF, traits_mask = 0xFFFFul
				<< traits_offset
	};
...
public:
	...
	enum traits_type {
		exact_exception = 0x0001ul << traits_offset, concurrent_wait = 0x0004ul
				<< traits_offset,
#if TBB_USE_CAPTURED_EXCEPTION
		default_traits = 0
#else
...
private:
	...
	char _leading_padding[internal::NFS_MaxLineSize - 2 * sizeof(uintptr_t)
			- sizeof(void*) - sizeof(internal::context_list_node_t)
			- sizeof(__itt_caller )];
	...
	/** \sa _leading_padding **/
	char _trailing_padding[internal::NFS_MaxLineSize - 2 * sizeof(uintptr_t)
			- 2 * sizeof(void*)
#if __TBB_TASK_PRIORITY
			- sizeof(intptr_t)
#endif /* __TBB_TASK_PRIORITY */
...
public:
	...
	task_group_context(kind_type relation_with_parent = bound,
			...
			my_kind(relation_with_parent), my_version_and_traits(1 | traits) {
		init();
	}
...
private:
	...
	void propagate_task_group_state(T task_group_context::*mptr_state,
			task_group_context& src, T new_state);
