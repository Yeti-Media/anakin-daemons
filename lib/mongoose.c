...
enum ns_event {
NS_POLL, NS_ACCEPT, NS_CONNECT, NS_RECV, NS_SEND, NS_CLOSE
};
...
struct ns_connection *ns_connect(struct ns_server *, const char *host, int port,
	int ssl, void *connection_param);
...
if (conn->prev == NULL)
	conn->server->active_connections = conn->next;
if (conn->prev)
...
if (conn->server->callback)
	conn->server->callback(conn, ev, p);
}
...
return server->listening_sock == INVALID_SOCKET ?
		-1 : (int) ntohs(server->listening_sa.sin.sin_port);
}
...
if (select((int) max_fd + 1, &read_set, &write_set, NULL, &tv) > 0) {
	...
	if (server->ctl[1] != INVALID_SOCKET && FD_ISSET(server->ctl[1], &read_set)) {
		unsigned char ch;
...
if (start_process(conn->server->config_options[CGI_INTERPRETER], prog, blk.buf,
	...
	conn->endpoint.cgi_conn = ns_add_sock(&conn->server->ns_server, fds[0],
			conn);
...
if ((or_str = (const char *) memchr(pattern, '|', pattern_len)) != NULL) {
	...
	return res > 0 ?
			res :
			mg_match_prefix(or_str + 1, (pattern + pattern_len) - (or_str + 1),
					str);
}
...
"Connection: %s\r\n"
"Accept-Ranges: bytes\r\n"
"%s%s\r\n", conn->mg_conn.status_code, msg, date, lm, etag, (int) mime_vec.len,
		mime_vec.ptr, conn->cl, suggest_connection_header(&conn->mg_conn),
...
while ((dp = readdir(dirp)) != NULL) {
	...
	if (arr_ind >= arr_size) {
		if ((p = (struct dir_entry *) realloc(*arr,
				(inc + arr_size) * sizeof(**arr))) != NULL) {
...
mg_printf_data(&de->conn->mg_conn, "<tr><td><a href=\"%s%s%s\">%s%s</a></td>"
		"<td>&nbsp;%s</td><td>&nbsp;&nbsp;%s</td></tr>\n",
...
mg_parse_header(mg_get_header(&conn->mg_conn, "Authorization"), "username",
		user, sizeof(user));
fprintf(fp, "%s - %s [%s] \"%s %s%s%s HTTP/%s\" %d %" INT64_FMT, c->remote_ip,
		user[0] == '\0' ? "-" : user, date,
...
if (!check_acl(server->config_options[ACCESS_CONTROL_LIST],
		...
		(struct connection *) calloc(1, sizeof(*conn))) == NULL) {
	...
	conn->mg_conn.local_port = ntohs(
			server->ns_server.listening_sa.sin.sin_port);
